          ...
          <CardContent>
            <Tabs defaultValue="all">
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="notices" className="text-blue-600">
                  ...
                  <div key={index} className="border rounded-lg p-4 space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">{issue.code}</p>
                      </div>
                    </div>
                    <div className="ml-6 space-y-1">
                      <p className="text-sm"><strong>Selector:</strong> {issue.selector}</p>
                      <details className="text-sm">
                        <summary className="cursor-pointer hover:underline">View context</summary>
              ...
              <TabsContent value="errors">
                {getIssuesByType('error').map((issue, index) => (
                  <div key={index} className="border border-destructive/50 rounded-lg p-4 space-y-2">
                    <div className="flex items-start gap-2">
                      {getIssueIcon(issue.type)}
                      <div className="flex-1">
                        <h4 className="font-medium">{issue.message}</h4>
              ...
              <TabsContent value="warnings">
                {getIssuesByType('warning').map((issue, index) => (
                  <div key={index} className="border border-yellow-500/50 rounded-lg p-4 space-y-2">
                    <div className="flex items-start gap-2">
                      {getIssueIcon(issue.type)}
                      <div className="flex-1">
                        <h4 className="font-medium">{issue.message}</h4>
              ...
              <TabsContent value="notices">
                {getIssuesByType('notice').map((issue, index) => (
                  <div key={index} className="border border-blue-500/50 rounded-lg p-4 space-y-2">
                    <div className="flex items-start gap-2">
                      {getIssueIcon(issue.type)}
                      <div className="flex-1">
                        <h4 className="font-medium">{issue.message}</h4>
